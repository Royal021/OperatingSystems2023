...
static void memdump(const void* p, int count){
...
void sweet(){
    const char* article_content[] = {article1, article2,article3,article4,article5,article6};
    const char* filenames[6] = { "article1.txt","article2.txt","article3.txt","article4.txt","article5.txt","article6.txt"};
    unsigned lengths[6];
    ...
    int fds[6];
    for(int i=0;i<6;++i){
        fds[i] = file_open(filenames[i],0);
        if(fds[i] < 0 ){
            kprintf("testsuite: Could not open %s (%d)\n",filenames[i],fds[i]);
            return;
        }
    }
    ...
    char* p[6] = {article1_, article2_, article3_, article4_, article5_, article6_ };
    ...
    while( 1 ){
        ...
        for(int j=0;j<6;++j){
            ...
            if( rv > readsize ){
                kprintf("testsuite: Too much data read for %s: Got %d, expected %d or less\n",filenames[i],rv,readsize);
                return;
            }
            count[i] += (unsigned)rv;
            if( count[i] > lengths[i] ){
                kprintf("testsuite: Did not reach EOF for %s\n",filenames[i]);
                return;
            }
            if( rv == 0 )
                atEOF++;
    ...
    for(int i=0;i<6;++i){
        if( count[i] != lengths[i] ){
            kprintf("testsuite: Incorrect data for %s: Got %d, expected %d\n",
                filenames[i], count[i], lengths[i] );
            return;
        }
        if(VERBOSE){
            vprintf("testsuite: Contents of %s:\n",filenames[i]);
            memdump(p[i],(int)count[i]);
        }

        for(int j=0;j<count[i];++j){
            if( p[i][j] != article_content[i][j] ){
                kprintf("Mismatch in %s at index %d\n",filenames[i],j);
                int start = j-40;
                if( start < 0 )
                    start = 0;
                int end = j+40;
                ...
                kprintf("Expected:\n");
                memdump(article_content[i]+start, end-start);
                kprintf("\nGot:\n");
                memdump(p[i]+start,end-start);
                return;
            }
        }
    }
